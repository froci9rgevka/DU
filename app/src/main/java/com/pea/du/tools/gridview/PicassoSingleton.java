...
import static com.squareup.picasso.Cache.NONE;

//Singleton Class for Picasso Downloading, Caching and Displaying Images Library
public class PicassoSingleton {

    private static Picasso mInstance;
    private static long mDiskCacheSize = 200*1024*1024; //Disk Cache limit 50mb
    ...
    public static synchronized Picasso getSharedInstance(Context context)
    {
        if(mInstance == null) {
            if (context != null) {
                ...
                mInstance = new Picasso.Builder(context).memoryCache(NONE).downloader(new OkHttp3Downloader(mOkHttp3Client)).indicatorsEnabled(true).build();
